...
class NFCmethods(object):
    ...
    @staticmethod
    def bytes_to_hex(databytes, kind = "long"):
        """to hex method like in smartcard utils, except keeping the 0X prefixes and add as array or with the kind parameter to short without the 0X prefixes"""
        datahex = []

        for i in databytes:
            i_hexstring = hex(i)
            if len(i_hexstring) == 3:
                i_hexstring = f"{i_hexstring[0:2]}0{i_hexstring[2]}"
            if kind == "short":
                i_hexstring = i_hexstring[2:]
...
class NDEFinterpreter(object):
    ...
    @staticmethod
    def decode_message(response_hex):

        print(f"trying to decode {response_hex}")
        message = ""
        for i in response_hex:
            print(f"i = {i}")
        ...
        decoded_message = bytes.fromhex(message).decode('latin-1')
        print(f"decoded message: {decoded_message}")
...
class NFCconnection(object):
    ...
    def get_card_data(self, length):
        ...
        page = 4
        while page >=4 and page < 15:
            response, response_hex, response_decoded = self.get_card_page(page, length)

            responses += [response]
            responses_hex += [response_hex]

            page += 1              

        print(f"responses of whole card is: {responses}")
        print(f"responses in hex of whole card is: {responses_hex}")

        responses_decoded = NDEFinterpreter.decode_bytes(responses_hex)
        print(f"response decoded: {responses_decoded}")

        print(f"responses decoded of whole card is: {responses_decoded}")

        return responses, responses_hex, responses_decoded
    ...
    def get_card_page(self, page, length):
        ...
        response_hex = NFCmethods.bytes_to_hex(response, "short")
        print(f"response hex: {response_hex}")

        return response, response_hex
